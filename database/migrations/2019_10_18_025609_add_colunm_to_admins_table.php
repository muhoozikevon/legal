    ...
    public function up()
    {
        Schema::table('admins', function (Blueprint $table) {
            //
             ...
             $table->enum('is_user_type',['SUPERADMIN','ADVOCATE','STAFF'])->default('ADVOCATE')->after('is_account_setup');
             $table->enum('invitation_status',['accepted','sent'])->default('sent')->after('is_user_type');
             ...
             $table->enum('is_active',['Yes','No'])->default('Yes')->after('last_login_ip');
             $table->enum('is_expired',['Yes','No'])->default('No')->after('is_active');
             $table->string('otp')->nullable()->after('is_expired');
             $table->timestamp('otp_date')->nullable()->after('otp');
             $table->enum('is_otp_verify',['0','1'])->default('0')->after('otp_date');
             $table->enum('plateform',['App','Web'])->default('Web')->after('is_otp_verify');
             $table->enum('user_type',['Admin'])->after('plateform');
